...
dvar int n[SFCInput][nBWVMs] in 0..1;
dexpr int AcceptedSFC = sum(i in SFCInput) ((prod(j in nBWVMs) n[i][j])*(prod(j in VNFs) (sum(s in Servers) VNFsuccessSer[i][j][s]
						+ sum(ed in EdgeDevs) VNFsuccessEdge[i][j][ed])));						
 ...
 forall(i in SFCInput, j in VNFs: j <= nBWVM, ed in EdgeDevs) // link occurs between edge & server
  ...
  forall(l1 in Links, l2 in Links: l1.des == l2.org && l1.org == l2.des) // cap bandwidth of link l cannot be exceeded
    sum(i in SFCInput, j in nBWVMs)
      (x[i][j][l1] + x[i][j][l2])*bwVM[j] <= capBWServer;
      
  forall(gw in GW) // customize for link from edge --> gw
    forall(l in Links: l.des == gw)
      sum(i in SFCInput, j in nBWVMs)
        x[i][j][l]*bwVM[j] <= capBWEdge;
  
  forall(i in SFCInput, j in nBWVMs) // prevent looping stage 1
	  { sum(e1 in EdgeSwitches, e2 in Nodetype1,l in Links: l.org == e1 && l.des == e2) x[i][j][l] <= 1;
	  	sum(e1 in EdgeSwitches, e2 in Nodetype1,l in Links: l.org == e2 && l.des == e1) x[i][j][l] <= 1;}

  forall(i in SFCInput, j in nBWVMs) // prevent looping stage 1
	  { sum(e1 in Nodetype1, e2 in Nodetype2, l in Links: l.org == e1 && l.des == e2) x[i][j][l] <= 1;
	  	sum(e1 in Nodetype1, e2 in Nodetype2, l in Links: l.org == e2 && l.des == e1) x[i][j][l] <= 1;}
  ...
  forall(i in SFCInput)
    forall(j in nBWVMs)
      n[i][j] == sum(s in Servers) (VNFsuccessSer[i][j][s] * VNFsuccessSer[i][j+1][s]) +
      		sum(ed in EdgeDevs) (VNFsuccessEdge[i][j][ed] * VNFsuccessEdge[i][j+1][ed]) +
